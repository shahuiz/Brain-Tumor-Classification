...
from vit_model import vit_large_patch16_224_in21k as creat_model_large_ep10
from vit_model import vit_base_patch16_224_in21k as creat_model_base_ep10

def main():
    
    test_images_path, test_images_label = read_data(root = "C:\\Users\\zhang\\Desktop\\BTC\\data\\Testing")
    length = len(test_images_path)

    truth_table = pd.DataFrame(0, columns=['glioma_tumor', 'meningioma_tumor', 'no_tumor', 'pituitary_tumor'], index = ['glioma_tumor', 'meningioma_tumor', 'no_tumor', 'pituitary_tumor'])
...
def vit_classifier(img_path: str, img_label: str, iter: int):
    ...
    # create model
    model = creat_model_large_ep10(num_classes=4, has_logits=False).to(device)

    # load model weights
    model_weight_path = "C:\\Users\\zhang\\Desktop\\BTC\\ViT\\models\\vit_large_patch16_epoch10_weights\\model-9.pth"
